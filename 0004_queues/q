...
if [ -z "$1" ]; then
    ...
    echo ""
    echo " If you do not see result in 120 seconds, please restart the Jupyter kernel:"
    ...
    until [ -f $script.o$job_id ]; do
        ...
        echo -ne "█"
        ((timeout++))
        # Timeout if no output file generated within 120 seconds
        if [ $timeout == 120 ]; then
            echo ""
            echo ""
            echo "TimeOut 120 seconds: Job is still queued for execution, check for output file later ($script.o$job_id)"
            echo ""
            break
        fi
    done
